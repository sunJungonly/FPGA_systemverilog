...
module guided_filter_top #(
    ...
    logic [11:0] guide_pixel_gray, guide_pixel_gray_d1;
    ...
    logic [ 7:0] input_pixel_in_d1;

    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            input_pixel_in_d1 <= 0;
            guide_pixel_gray_d1 <= 0;
        end
        else begin
            input_pixel_in_d1 <= input_pixel_in;
            guide_pixel_gray_d1 <= guide_pixel_gray;
        end
    end
    ...
    always_ff @( posedge clk or posedge rst ) begin
        if(rst)begin
            ...
            i_mul_p_reg <= guide_pixel_gray_d1 * input_pixel_in_d1;
        end
    end

    assign i_mul_p = i_mul_p_reg;
    ...
    Final_Output_Calculator #(
        ...
        .rst(rst),
        .DE(DE),
        .guide_pixel_gray(guide_pixel_gray_d1),
        .sum_a(sum_a),
        .sum_b(sum_b),
        .valid_a(valid_a),
        .q_i(q_i),
...
module window_sum_calculator #(
    ...
    parameter DUMMY = {DATA_WIDTH{1'b0}}
) (
    input  logic                    clk,
    input  logic                    rst,
    ...
    logic [COL_SUM_SQ_WIDTH-1:0] temp_sum_sq, sum_square;
    ...
    always_ff @( posedge clk or posedge rst ) begin
        if (rst)begin
            ...
            for (int k = 0; k < WINDOW_SIZE; k++) begin
                ...
                sum_square <= col_data_out_reg[k] * col_data_out_reg[k];
                temp_sum_sq <= temp_sum_sq + sum_square;
            end
    ...
    generate
        for(i = 0; i < WINDOW_SIZE-1; i = i + 1)begin
            data_mem1 # (
                ...
                .wen(counter == 2),
                .waddr(x_pixel_ram),
                .wdata(DATA_WIDTH'(x_pixel >= 639) ? DUMMY : (i==0) ? pixel_in : ram_chain_out[i-1]),
                .ren(1'b1),
                .raddr(x_pixel_ram),
    ...
    assign sum_out = (window_sum * 7282) >>> 19; // 7282 / 2^19 ≈ 1 / 225
    assign sum_sq_out = (window_sum_sq * 7282) >>> 19; // 7282 / 2^19 ≈ 1 / 225
...
module AB_Calculator #(
    ...
    logic [63:0] quotient_from_ip, quotient_from_ip_d1;
    logic [63:0] remainder_from_ip;
    logic [63:0] quotient_from_ip_int;
    logic [ 7:0] quotient_from_ip_frac;
    ...
    shift_divider #(
        ...
        // 결과(Quotient) 출력
        .quotient_integer_out (quotient_from_ip_int), // 몫 정수부
        .quotient_fractional_out (quotient_from_ip_frac), // 몫 소수부
        .remainder_out_tdata(remainder_from_ip), // 나머지
        .quotient_out_tvalid (quotient_valid)
    );

    // assign quotient_from_ip_d1 = {quotient_from_ip_int{63:44}};
    ...
    logic signed [SUM_P_WIDTH + AK_FRAC_BITS:0] b_k_num, mul_result;
    always_ff @( posedge clk or posedge rst) begin
        if(rst)begin
            mul_result <= 0;
            b_k_num <= 0;
            quotient_from_ip_d1 <= 0;
        end
        else if(quotient_valid)begin // a_k 계산 완료
            quotient_from_ip_d1 <= quotient_from_ip;
            mul_result <= $signed(quotient_from_ip_int) * sum_i_temp;
            b_k_num <= (sum_p_temp << AK_FRAC_BITS) - mul_result; 
        end
    end

    assign b_k_out = b_k_num >>> ($clog2(N) + AK_FRAC_BITS);
    assign a_k_out = quotient_from_ip_int[63:44]; // 4bit 정수부 + 16bit 실수부
