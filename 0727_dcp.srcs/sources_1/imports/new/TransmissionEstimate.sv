...
module TransmissionEstimate #(
    ...
    logic [15:0] quotient_integer_out;  // 나눗셈 몫
    // logic [15:0] quotient_fractional_out;         // 나눗셈 몫
    ...
    shift_divider #(
        ...
        // 분자(Dividend) 입력
        .divided_in_tdata (signed'(s1_omega_mul_dc)),
        .divided_in_tvalid(s2_tvalid_in),
        .divided_in_tready(s2_tready_out),

        // 분모(Divisor) 입력
        .divisor_in_tdata(signed'({8'b0, airlight_representative})),  //이게 맞다!!!! 대신 0.01정도의 오차 있음
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (output_valid) begin
                ...
                t_out <= 8'd255 - unsigned'(quotient_integer_out);
