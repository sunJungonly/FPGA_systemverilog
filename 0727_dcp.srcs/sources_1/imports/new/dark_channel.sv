...
module dark_channel (
    ...
    logic [11:0] DE_pipe;
    logic [11:0] h_sync_pipe;
    logic [11:0] v_sync_pipe;
    ...
    always_ff @(posedge pclk or posedge rst) begin
        if (rst) begin
            ...
            DE_pipe     <= {DE_pipe[10:0], src_block_min_DE};
            h_sync_pipe <= {h_sync_pipe[10:0], h_sync_out1};
            v_sync_pipe <= {v_sync_pipe[10:0], v_sync_out1};
        end
    end

    assign DE_out           = DE_pipe[11];
    assign h_sync_out       = h_sync_pipe[11];
    assign v_sync_out       = v_sync_pipe[11];

    assign dark_channel_out = src_block_min_img;
    assign x_pixel_out      = src_block_min_x_pixel;
    assign y_pixel_out      = src_block_min_y_pixel;
endmodule
