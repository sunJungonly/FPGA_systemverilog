...
module guided_filter_top (
    ...
    logic [27:0] window_sum_i;
    logic [31:0] window_sum_ii;
    logic [23:0] window_sum_p;
    logic [35:0] window_sum_ip;
    ...
    //     .red_port({3'b0, guide_pixel_in[15:11]}),
    //     .green_port({2'b0, guide_pixel_in[10:5]}),
    //     .blue_port({3'b0, guide_pixel_in[4:0]}),
    ...
    window_sum #(
        ...
        .pixel_in(a_k_out),
        .DE(DE_pipe[71]),
        .x_pixel(x_pixel_pipe[71]), // 75
        .y_pixel(y_pixel_pipe[71]), // 75
        .mean_out(sum_a),
        .valid_out()
    );
    ...
    window_sum #(
        ...
        .pixel_in(b_k_out),
        .DE(DE_pipe[71]),
        .x_pixel(x_pixel_pipe[71]),
        .y_pixel(y_pixel_pipe[71]),
        .mean_out(sum_b),
        .valid_out()
    );
    ...
    Final_Output_Calculator #(
        ...
        .rst(rst),
        .DE(DE_pipe[98]),
        .guide_pixel_gray(guide_pixel_gray_pipe[95]), // guided gray는 2clk delay더 있는거라서
        .mean_a(sum_a),
        .mean_b(sum_b),
        .valid_a(DE_pipe[96]),
        .q_i(q_i),
...
module window_sum_calculator #(
    ...
    input  logic [9:0]              x_pixel,
    input  logic [9:0]              y_pixel,
    output logic [SUM_WIDTH-1:0]    mean_out,
    output logic [SUM_SQ_WIDTH-1:0] mean_sq_out,
    ...
    logic [COL_SUM_WIDTH+6:0] temp_sum, temp_sum_reg;
    ...
    logic [2*DATA_WIDTH-1:0] sq [0:WINDOW_SIZE-1];
    genvar k;
    generate
        for (k=0; k<WINDOW_SIZE; k++) begin
            always_ff @(posedge pclk) begin
                sq[k] <= col_data_out_reg[k] * col_data_out_reg[k];
            end
        end
    endgenerate

    localparam SUMW  = COL_SUM_WIDTH+6;
    localparam SQW   = COL_SUM_SQ_WIDTH;

    logic [SUMW-1:0]  s1 [0:7];
    logic [SQW-1:0]   s1_sq [0:7];

    always_ff @(posedge pclk) begin
        s1[0] <= col_data_out_reg[0]  + col_data_out_reg[1];
        s1[1] <= col_data_out_reg[2]  + col_data_out_reg[3];
        s1[2] <= col_data_out_reg[4]  + col_data_out_reg[5];
        s1[3] <= col_data_out_reg[6]  + col_data_out_reg[7];
        s1[4] <= col_data_out_reg[8]  + col_data_out_reg[9];
        s1[5] <= col_data_out_reg[10] + col_data_out_reg[11];
        s1[6] <= col_data_out_reg[12] + col_data_out_reg[13];
        s1[7] <= col_data_out_reg[14];
    ...
    logic [SUMW-1:0]  s2 [0:3];
    logic [SQW-1:0]   s2_sq [0:3];
    ...
    always_ff @(posedge pclk) begin
        ...
        s2_sq[0] <= s1_sq[0] + s1_sq[1];
        s2_sq[1] <= s1_sq[2] + s1_sq[3];
        s2_sq[2] <= s1_sq[4] + s1_sq[5];
        s2_sq[3] <= s1_sq[6] + s1_sq[7];
    end

    logic [SUMW-1:0]  s3 [0:1];
    logic [SQW-1:0]   s3_sq [0:1];

    always_ff @(posedge pclk) begin
        s3[0]   <= s2[0]   + s2[1];
        s3[1]   <= s2[2]   + s2[3];

        s3_sq[0] <= s2_sq[0] + s2_sq[1];
        s3_sq[1] <= s2_sq[2] + s2_sq[3];
    end

    always_ff @(posedge pclk) begin
        temp_sum    <= s3[0] + s3[1];
        temp_sum_sq <= s3_sq[0] + s3_sq[1];
    end
    ...
    localparam signed [MEAN_FRAC_BITS:0] INV_N = (1<<MEAN_FRAC_BITS)/N;
...
module AB_Calculator #(
    ...
    localparam AK_CAL_LATENCY = 1 + DIVIDER_LATENCY; // sum ?  ?   ??   a_k 계산 ?  료까 ??    ?  ??  ?   ? (mul/sub + divider)
    ...
    // assign dividend_reg = $signed(cov_num_reg) <<< AK_FRAC_BITS;
    ...
    logic signed [19:0] a_k_pipe[0:1];
    ...
    logic signed [32:0]  mul_result_temp;
    logic valid_delay;
    logic nDivide_sig;
...
module Final_Output_Calculator #(
    ...
    assign q_i = (q_i_final < 0) ? 8'd0 : (q_i_final > 255) ? 8'd240: q_i_final[7:0];
...
module window_sum #(
    ...
    input  logic [9:0]              x_pixel,
    input  logic [9:0]              y_pixel,
    output logic signed [SUM_WIDTH-1:0]    mean_out,
    output logic                    valid_out
);
    ...
    always_ff @( posedge pclk or posedge rst ) begin
        ...
        else if((x_pixel != x_pixel_d1) && (x_pixel > 4 && x_pixel < 661)) begin
            if(x_pixel < 20) begin
                window_sum <= window_sum + col_sum;
            end
            else if(x_pixel > 645) begin
                window_sum <= window_sum - oldest_col_sum;
            end
            else begin
                window_sum <= window_sum + col_sum - oldest_col_sum;
