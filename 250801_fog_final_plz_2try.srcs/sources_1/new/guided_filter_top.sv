...
module guided_filter_top (
    ...
    logic [11:0] guide_pixel_gray_pipe[0:97];
    logic [ 7:0] input_pixel_in_pipe[0:97];
    ...
    window_sum #(
        ...
        .pixel_in(a_k_out),
        .DE(DE_pipe[77]),
        .x_pixel(x_pixel_pipe[77]),
        .y_pixel(y_pixel_pipe[77]),
        .mean_out(sum_a),
        .mean_sq_out(), // dummy
        .valid_out()
    );
    ...
    Final_Output_Calculator #(
        ...
        .rst(rst),
        .DE(DE_pipe[97]),
        .guide_pixel_gray(guide_pixel_gray_pipe[97]), // 충분?   delay?  줘야 ?  
        .mean_a(sum_a),
        .mean_b(sum_b),
        .valid_a(DE_pipe[97]),
        .q_i(q_i),
...
module window_sum_calculator #(
    ...
    localparam COL_SUM_WIDTH = DATA_WIDTH + $clog2(WINDOW_SIZE) + 2;
    localparam COL_SUM_SQ_WIDTH = DATA_WIDTH*2 + $clog2(WINDOW_SIZE) + 2;
        ...
        if (rst)begin
            ...
            if(counter == 2) begin
                if(y_pixel >= 14)begin
                    ...
                    temp_sum <= temp_sum + col_data_out_reg[0];
                ...
                sum_square[0] <= col_data_out_reg[0] * col_data_out_reg[0];
    ...
    logic [SUM_WIDTH+6:0]        window_sum;
    ...
    always_ff @( posedge clk or posedge rst ) begin
        if(rst)begin
            ...
            mul_result <= window_sum * INV_N + (1 << 17);
...
module AB_Calculator #(
    ...
    always_ff @( posedge clk or posedge rst ) begin
        ...
        else if(valid_in) begin // ?  ?  ?   sum 값이 ?  ?  ?   ?  마다 ?  ?  ?  ?  ?  ?   ?  ?  
    ...
    // ?  ?  ?  ?  ?  ?   ?  과하?   a_k?? ???  밍이 ?  기화?   sum_i, sum_p  ?
    logic [SUM_I_WIDTH-1:0] sum_i_synced;
    logic [SUM_P_WIDTH-1:0] sum_p_synced;
    ...
    always_ff @( posedge clk or posedge rst) begin
        if(rst)begin
            ...
            if(quotient_valid)begin // a_k 계산?   ?  료된 ?  ?  ?  
                // [?  ?   1] ?  바른 a_k 값과 [?  ?   2] ?  기화?   sum 값으 ? ?  ?  
                
                b_k_num <= sum_p_synced - mul_result;
            end
        end
    end

    // b_k = b_k_num / (N * 2^16) ?   ? ?, ?  ?  ?  ?   친화?  ?   ? (b_k_num / 2^clog2(N)) / 2^16 ?   ? 근사
...
module Final_Output_Calculator #(
    ...
    logic signed [GRAY_WIDTH:0]                          q_i_final;

    assign mul_q_result = mean_a * guide_pixel_gray;

    assign q_i_final = (mul_q_result >>> AK_FRAC_BITS) + mean_b;
...
module window_sum #(
    ...
    localparam N = WINDOW_SIZE * WINDOW_SIZE;
    localparam COL_SUM_WIDTH = DATA_WIDTH + $clog2(WINDOW_SIZE) + 8;
    localparam COL_SUM_SQ_WIDTH = DATA_WIDTH*2 + $clog2(WINDOW_SIZE) + 8;
    ...
    logic signed [COL_SUM_WIDTH+6:0] temp_sum, temp_sum_reg; // COL_SUM_WIDTH-1
    logic signed [COL_SUM_SQ_WIDTH:0] temp_sum_sq;
    logic signed [COL_SUM_SQ_WIDTH:0] sum_square [0:WINDOW_SIZE-1];
    ...
    logic signed [SUM_WIDTH+6:0]        window_sum;
