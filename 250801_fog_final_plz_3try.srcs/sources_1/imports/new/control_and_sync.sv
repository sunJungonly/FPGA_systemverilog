...
module control_and_sync #(
    ...
    parameter TE_LATENCY   = 21,
    parameter GU_LATENCY   = 28,
    ...
    // input logic h_blank_in,
    // input logic v_blank_in,
    ...
    // output logic h_blank_out,
    // output logic v_blank_out,

    // pixel location
    output logic [9:0] guided_x_pixel_out,
    output logic [9:0] guided_y_pixel_out,
    ...
    logic [9:0] guided_x_pixel_out_delayed [0:GUIDED_DELAY_DEPTH-1];
    logic [9:0] guided_y_pixel_out_delayed [0:GUIDED_DELAY_DEPTH-1];
    ...
    logic h_blank_delay [0:FINAL_DELAY_DEPTH-1];
    logic v_blank_delay [0:FINAL_DELAY_DEPTH-1];
    ...
    always_ff @(posedge pclk or posedge rst) begin
        if (rst) begin
            ...
            for (int i = 0; i < GUIDED_DELAY_DEPTH; i = i + 1) begin
                ...
                guided_x_pixel_out_delayed[i] <= 0;
                guided_y_pixel_out_delayed[i] <= 0;
            end
            ...
            guided_x_pixel_out_delayed[0] <= x_pixel_in;
            guided_y_pixel_out_delayed[0] <= y_pixel_in;
            for (int i = 0; i < GUIDED_DELAY_DEPTH - 1; i++) begin
                guided_de_delayed[i+1] <= guided_de_delayed[i];
                guided_x_pixel_out_delayed[i+1] <= guided_x_pixel_out_delayed[i];
                guided_y_pixel_out_delayed[i+1] <= guided_y_pixel_out_delayed[i];
            end
            ...
            // h_blank_delay[0] <= h_blank_in;
            // v_blank_delay[0] <= v_blank_in;
            ...
            for (int i = 0; i < FINAL_DELAY_DEPTH - 1; i++) begin
                ...
                h_blank_delay[i+1] <= h_blank_delay[i];
                v_blank_delay[i+1] <= v_blank_delay[i];
    ...
    assign guided_x_pixel_out = guided_x_pixel_out_delayed[GUIDED_DELAY_DEPTH-1];
    assign guided_y_pixel_out = guided_y_pixel_out_delayed[GUIDED_DELAY_DEPTH-1];    
    ...
    assign h_blank_out = h_blank_delay[FINAL_DELAY_DEPTH-1];
    assign v_blank_out = v_blank_delay[FINAL_DELAY_DEPTH-1];
