...
module TransmissionEstimate #(
    ...
    // --- Stage 2: 나눗셈 IP 연결 ---
    logic [31:0] div_full_quotient_raw;  // 나눗셈 결과(몫 + 나머지)
    logic [15:0] div_quotient_raw;  // 나눗셈 결과
    ...
    divider_IP_wrapper u_divider (
        ...
        // 분자(Dividend) 입력
        .divided_in_tdata(omega_mul_dc),  //16bit // 1단계 연산 결과
        .divided_in_tvalid(DE_s1),  // 1단계 데이터가 유효함을 알림
        .divided_in_tready(div_ready_in),  // IP가 받을 준비가 되었는지 (Back-pressure)

        // 분모(Divisor) 입력
        .divisor_in_tdata({8'd0, airlight_representative}),  //8bit
        .divisor_in_tvalid(DE_s1),
        .divisor_in_tready(),  // 일반적으로 분자는 tready만 사용

        // 결과(Quotient) 출력
        .divided_out_tdata(div_full_quotient_raw), // IP 출력 포맷에 맞게 연결 (하위 비트는 버림)
        .divided_out_tvalid(div_valid_out_raw)
    );

    assign div_quotient_raw = div_full_quotient_raw[31:16];

    // --- Stage 3: 1 - (나눗셈 결과) & 최종 출력 ---
    // 나눗셈 결과와 제어 신호를 동기화
    logic [DIVIDER_LATENCY-1:0] DE_delay_chain;
    logic [DIVIDER_LATENCY-1:0][8:0] x_pixel_delay_chain;
